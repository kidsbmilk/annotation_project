...
public class Channels {
    ...
    public static ChannelPipeline pipeline(ChannelPipeline pipeline) { // 克隆现有的pipeline，并返回克隆出的新的pipeline。
    ...
    /**
     * 下面的fireChannelXXX函数都有两种：
     * 1、调用getPipeline().sendUpstream的传递方式，都是从head开始查找执行的；
     * 2、调用ctx.sendUpstream的传递方式，是从ctx.next开始查找执行的。
     *
     * 这一部分都是调用的sendUpstream。
     */
    public static void fireChannelOpen(Channel channel) {
        if (channel.getParent() != null) { // 这个if分支不太明白 ?zz?
            fireChildChannelStateChanged(channel.getParent(), channel);
        }
        channel.getPipeline().sendUpstream(
                new DefaultChannelStateEvent(
                        channel, succeededFuture(channel),
                        ChannelState.OPEN, Boolean.TRUE)); // 这种调用getPipeline().sendUpstream的传递方式，都是从head开始查找执行的。
    }
    ...
    public static void fireChannelOpen(
        ...
        ctx.sendUpstream(new DefaultChannelStateEvent(
                channel, succeededFuture(channel),
                ChannelState.OPEN, Boolean.TRUE)); // 这种调用ctx.sendUpstream的传递方式，是从ctx.next开始查找执行的。
    }
    ...
    public static void fireChannelBound(Channel channel, SocketAddress localAddress) {
        channel.getPipeline().sendUpstream(
                new DefaultChannelStateEvent(
                        channel, succeededFuture(channel),
                        ChannelState.BOUND, localAddress)); // 看看这里的getPipeline()是啥 ?zz? 得到的还是ServerBootstrap#bind里创建的bossPipeline。
    }
    ...
    /**
     * 接下来是一些实用的操作函数，与上面的fireChannelXXX之类的不同，上面的都是sendUpstream，下面的都是sendDownstream。
     * 与fireChannelXXX之类的类似，也分为两部分：
     * 1、调用getPipeline().sendUpstream的传递方式，都是从head开始查找执行的；
     * 2、调用ctx.sendUpstream的传递方式，是从ctx.next开始查找执行的。
     */
