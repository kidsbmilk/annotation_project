...
public final class Futures extends GwtFuturesCatchingSpecialization {
  ...
  // 以及Future完成后，不需要再保留输入Future),所以设置为final是不合理的。（我明白为什么此前有些并发对象的成员变量要设置为final了。
    // final时统计器会禁止将final对象初始化在构造函数之外。见《并发编程的艺术》）
  // 将成员变量设置为volatile（也是禁止指令重排的）能在一定程序上解决此问题，这也是TODO里说的，但是依然存在一些小问题，见上面的2，因为它并不能看到最初的写。
  ...
  // 现在认识全面了也不算晚。
  //
  // jvm并发包中有没有些问题？还是说JVM没有实现这样的传播，所以不会存在此问题?
   ...
   * 取消返回的future时，会取消输入的所有future，如果输入的future有任何一个取消或者失败了，则也取消这个返回的future。
   *
