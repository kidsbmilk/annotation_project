 ...
 *
 * （google翻译：包装缓冲区是一个缓冲区，它是一个或多个现有字节数组和字节缓冲区的视图。
 * 原始数组或缓冲区内容的任何更改都将在包装缓冲区中可见。 提供了各种包装方法，
 * 它们的名字都是{@code wrappedBuffer（）}。 如果你想创建一个由多个数组组成的缓冲区来减少内存拷贝的数量，
 * 你可能想仔细看看接受可变参数的方法。）
 *
...
/**
 * 这个类中与数组相关的读写，与分散IO scatter/gather非常像。
 */
public final class Unpooled {
     ...
     * consolidation occurs.（关于在合并发生之前允许存在多少独立缓冲区的建议。）
     * @param buffers The buffers to wrap. Reference count ownership of all variables is transfered to this method.
     * @return The readable portion of the {@code buffers}. The caller is responsible for releasing this buffer.
     */
