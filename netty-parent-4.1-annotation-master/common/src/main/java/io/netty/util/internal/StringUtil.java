...
public final class StringUtil {
     ...
     *
     * 返回delim后面的字符串，如果没有找到delim，则返回null。
     */
    public static String substringAfter(String value, char delim) {
        int pos = value.indexOf(delim); // 这个indexOf返回delim第一次出现的位置
        if (pos >= 0) {
    ...
    public static String byteToHexStringPadded(int value) {
        return BYTE2HEX_PAD[value & 0xff]; // 只取最后一个字节
    }
    ...
    public static <T extends Appendable> T toHexString(T dst, byte[] src, int offset, int length) {
        ...
        byteToHexString(dst, src[i++]); // 只有第一个字符不用PAD形式的，其余的都是用PAD形式的，这样做是为了什么 ?zz?
        // 是不是作者写错了？？？
        int remaining = end - i;
     ...
     * @return {@link CharSequence} the escaped value if necessary, or the value unchanged
     */
    public static CharSequence escapeCsv(CharSequence value, boolean trimWhiteSpace) { // trimWhiteSpace表示是否去除前后的空白符
        ...
        if (isDoubleQuote(value.charAt(start))) { // 如果当前start处字符是个双引号，则判断当前last处是否是双引号
        ...
        if (firstUnescapedSpecial < 0) { // 当前没有单个的双引号
            if (quoted) { // 并且找到并跳过了一对双引号
                for (int i = start; i <= last; i++) {
                    if (isDoubleQuote(value.charAt(i))) {
                        if (i == last || !isDoubleQuote(value.charAt(i + 1))) { // 如果最后一个字符是双引号或者双引号不成对出现
                ...
                for (int i = start; i <= last; i++) {
                    ...
                    if (c == LINE_FEED || c == CARRIAGE_RETURN || c == COMMA) { // 如果字符是\n，\r，','，则标记firstUnescapedSpecial
                        firstUnescapedSpecial = i;
                        break; // 注意：找到就break
                    }
                    if (isDoubleQuote(c)) {
        ...
        for (int i = firstUnescapedSpecial; i <= last; i++) {
            // 这个for是：如果连着两个双引号，则效果是在result中加入两个双引号，如果是只有一个双引号，则也在result中加入两个双引号
            char c = value.charAt(i);
            if (isDoubleQuote(c)) {
            ...
            result.append(c); // 注意这一行！
        }
        return result.append(DOUBLE_QUOTE); // 与最开始头的双引号成对。
    }
    ...
    public static CharSequence unescapeCsv(CharSequence value) {
        ...
        for (int i = 1; i < last; i++) { // 跳过前后两个双引号
            char current = value.charAt(i);
            if (current == DOUBLE_QUOTE) {
                if (isDoubleQuote(value.charAt(i + 1)) && (i + 1) != last) { // 跳过成对的两个双引号
    ...
    public static List<CharSequence> unescapeCsvFields(CharSequence value) {
        ...
        for (int i = 0; i <= last; i++) {
            char c = value.charAt(i);
            if (quoted) { // 如果当前字符串被双引号包住了
                switch (c) {
                    case DOUBLE_QUOTE:
                        if (i == last) { // 如果当前已到字符串末尾且是双引号，则将current中的字符加入到unescaped中，并返回unescaped。
                            // Add the last field and return
                            unescaped.add(current.toString());
                            return unescaped;
                        }
                        char next = value.charAt(++i);
                        if (next == DOUBLE_QUOTE) { // 如果当前出现了一对双引号，则将一个双引号加入到current中。
            ...
            } else { // 一开始quoted等于false
                switch (c) {
                    case COMMA:
                        // Start to parse the next field
                        unescaped.add(current.toString());
                        current.setLength(0); // 重置这个StringBuilder ?zz?
                        break;
                    case DOUBLE_QUOTE:
                        if (current.length() == 0) { // 当current为空，且当前字符是双引号时
                            quoted = true;
                            break;
                        }
                    ...
                    case CARRIAGE_RETURN:
                        ...
                        throw newInvalidEscapedCsvFieldException(value, i); // 如果是回车或者换行，则抛出异常
                    default:
                        current.append(c); // 如果是正常的字符，则加入到current中。
                }
            }
        }
        if (quoted) {
