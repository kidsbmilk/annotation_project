...
/**
 * Light-weight object pool based on a thread-local stack（注意这个thread-local stack，thread-local体现在使用了FastThreadLocal，stack是这里自定义的Stack类）.
 *
 * @param <T> the type of the pooled object
 */
public abstract class Recycler<T> { // Recycler本意是反复循环器，看上面的类注释，这个类是轻量级的对象池的封装。
    ...
    static final class Stack<T> {
        ...
        // 我们保留每个线程队列的队列，每个队列只附加一次，每当一个新的线程而不是栈所有者回收时：当我们的堆栈(此新线程的堆栈，而不是栈的所有者的堆栈）中的项目用完时，
        // 我们迭代这个集合来清除那些可以被重用的。 这允许我们引起最小的线程同步，同时仍然回收所有物品。
