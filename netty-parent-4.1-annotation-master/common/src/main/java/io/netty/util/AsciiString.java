...
/**
 * 类的定义太规范了，构造函数的设计，具体方法的实现，都非常规范，只是方法比较多而已，并不难。
 */
...
public final class AsciiString implements CharSequence, Comparable<CharSequence> {
    ...
    public AsciiString(byte[] value, int start, int length, boolean copy) {
        if (copy) {
            ...
            if (isOutOfBounds(start, length, value.length)) {
                ...
                // 这是一个免检异常，不需要使用try-catch来捕获处理或者在方法头进行声明。
            }
            this.value = value;
            this.offset = start;
        }
     ...
     */
    public int forEachByte(ByteProcessor visitor) throws Exception { // 声明异常
     ...
     *         The last-visited index If the {@link ByteProcessor#process(byte)} returned {@code false}.
     */
    public int forEachByte(int index, int length, ByteProcessor visitor) throws Exception { // 声明异常
    ...
    private int forEachByte0(int index, int length, ByteProcessor visitor) throws Exception { // 声明异常
    ...
    public int parseInt(int start, int end, int radix) {
        ...
        if (negative && ++i == end) {
            throw new NumberFormatException(subSequence(start, end, false).toString());
            // NumberFormatException继承自IllegalArgumentException，是免检异常，不需要使用try-catch来捕获处理或者在方法头进行声明。
        }
