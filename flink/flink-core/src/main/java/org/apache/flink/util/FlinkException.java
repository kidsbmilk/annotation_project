 ...
 *
 * 在这里，FlinkException继承自Exception，并不是继承自RuntimeException。
 * 继承自RuntimeException的异常是unchecked异常，而checked异常并没有一个跟RuntimeException同级的父类，
 * 所以，要自定义checked异常的话，只能继承自Exception。
 * 从这一点，可以看出：unchecked只是异常中的一部分，而大部分异常都是需要捕获的。
 */
@Public
