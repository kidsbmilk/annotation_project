...
public class OptionalFailure<T> implements Serializable {
	 ...
	 *
	 * 注意：这里为什么只有一个CheckedSupplier版本的createFrom，而没有unchecked的Supplier版本的createFrom，
	 * 原因在于：OptionalFailure是函数式里处理异常的机制（以前看过相应的文章），它封闭了值与异常，就要确保能捕获任何异常，
	 * 所以，只有一个CheckedSupplier版本的createFrom。
	 *
	 * 参考AccumulatorHelper.mergeInto里的实现与注释。
	 */
